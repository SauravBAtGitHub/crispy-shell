<# 
.SYNOPSIS 
    Script to automate querying all ARM storage account in a given subscription for file shares and listing the share, directory and filenames along with their sizes. It also exports the files and their sizes to a C:\temp\<StorageAccountName>.csv file
...
##Looping through all ARM storage accounts in selected Subscription
$StorageObject = Get-AzureRMStorageAccount | Where-Object {$_.kind -ne 'BlobStorage'} | Select ResourceGroupName, StorageAccountName
